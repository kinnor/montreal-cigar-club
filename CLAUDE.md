...
- Domains (all Cloudflare Registrar): **`montrealcigarclub.ca`** (primary); **`mtlcigarclub.ca`**, **`mtlcigarclub.com`** (2026-08-29) and **`montrealcigarclub.com`** (2026-08-30) all 301 → primary via the redirect Worker. Hosting: Cloudflare Pages. Email Routing ACTIVE: `concierge@`, `admissions@`, `vault@` → rossen.kinov@gmail.com; outbound Email Sending ACTIVE (REST, `EMAIL_API_TOKEN`). Preview locally with `python -m http.server 8080`.
